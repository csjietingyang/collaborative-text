...
class GRCNN():
    ...
    def loss(self, input, gt):
        ...
        input_x1, input_x2, input_x3, input_y1, input_y2, input_y3 = self.loss_conv2d(input)
        gt_x1, gt_x2, gt_x3, gt_y1, gt_y2, gt_y3 = self.loss_conv2d(gt)
    ...
    def train(self):
        ...
        with tf.Graph().as_default():
            ...
            GRCNN_output = self.inference(GRCNN_input, Tar_edge)
            GRCNN_output_clip = tf.clip_by_value(GRCNN_output, 0.0, 255.0)

            _, tf_gr_in, tf_tar_edge, tf_tar_image = buildImage.decodeTfrecord()
            ...
            loss = self.loss(GRCNN_output, Tar_image)
            ...
            with tf.Session() as sess:
                ...
                learning_rates[10:] = learning_rates[0] / 10.0
                ...
                for epoch in range(epoch_cur, self.epochs):
                    ...
                    for batch in range(0, batch_num):
                        gr_input, target_edge, target_image = sess.run([tf_gr_in, tf_tar_edge, tf_tar_image])
                        _, lost, summary = sess.run([opt, loss, merged], feed_dict={GRCNN_input: gr_input, Tar_edge: target_edge, Tar_image: target_image, lr: learning_rates[epoch]})
                        all_lost += lost
    ...
    def test(self, model):
        ...
        with tf.Graph().as_default():
            GRCNN_input = tf.placeholder(shape=[None, test_down_size, test_down_size, 3], dtype=tf.float32)
            ...
            var_list = [v for v in tf.all_variables() if v.name.startswith('GRCNN')]
            ...
            with tf.Session() as sess:
                ...
                for i in range(test_num):
                    _, gr_input, target_edge, target_image = im2tfrecord.generatingSyntheticEdge(os.path.join(test_input_path, test_input_list[i]),
                    ...
                    output = sess.run(GRCNN_output, feed_dict={GRCNN_input: gr_input, Tar_edge: target_edge})
                    output = np.squeeze(output)
                    target_image = np.squeeze(target_image)
                    output = output.astype('uint8')
                    target_image = target_image.astype('uint8')

                    psnr_file[i] = psnr(output, target_image)
                    ssim_file[i] = ssim(output, target_image)

                    save_name = test_input_list[i].split('.')[0][:-5]
                    cv2.imwrite(os.path.join(save_path, save_name + '_output.png'), output)
...
if __name__ == '__main__':
    ...
    GRCNN_network = GRCNN(args)
    GRCNN_network.train()
    GRCNN_network.test(model='model_weight_12.ckpt')
